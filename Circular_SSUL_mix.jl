...
# elements, nodes, nodes_s, Ω, sp, type= import_Circular_mix("msh/circular/circular_quad8_"*string(ndiv)*".msh","msh/circular/circular_quad8_q_"*string(ndivs)*".msh");
nᵇ = length(nodes)
nˢ = length(nodes_s)
E = 10.92
ν = 0.3
...
s = 3.0*5/(ndivs)*ones(length(nodes_s))
